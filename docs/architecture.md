...
La composición Spring de la frontera interna de evaluaciones queda en
`AgentEvaluationTriggerConfiguration`. Registra el servicio de autorización,
el servicio de ejecución y un `AgentEvaluationTriggerAuthorizer` fail-closed
con `@ConditionalOnMissingBean`; un proveedor técnico explícito podrá
reemplazarlo en una evolución posterior sin modificar la aplicación.
...
no-op cuando `wcs.conversation.preferences.enabled=false`. El servicio valida
