...
| `AI_USAGE_RECORDED` | `stage`, `operation`, `provider`, `model`, `reasoningEffort` (cuando aplique al perfil Bedrock), `success`, `inputTokens`, `outputTokens`, `totalTokens`, `estimatedCostUsd`, `pricingVersion`, `durationMs`, `providerLatencyMs`, `promptVersion`, `promptHash`, `agentId`, `agentVersion`, `agentSemanticVersion`, `correlationId`, `errorType` | Cada llamada real a un proveedor de IA, correlacionable con la decisión sin registrar contenido |
...
| `CONVERSATIONAL_PURCHASE_LINK` | `operation`, `result`, `correlationId`, `channel`, `actorKey` opcional | Resultado de solicitar un checkout desde una conversación; nunca incluye URL, SKU, precio, teléfono ni contenido |
...
aprobado y un runbook de rollout. WCS-103 agrega `comparisonOutcome` con los
valores `MATCH`, `MISMATCH` o `UNKNOWN`. El hash SHA-256 de la respuesta activa
y candidata se calcula sólo en memoria; no se persiste ni se emite en logs.
WCS-104 agrega el adapter Bedrock sólo cuando
`wcs.agent-runtime.shadow-provider=bedrock`; recibe filtros de catálogo
