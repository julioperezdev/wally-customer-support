...
function CatalogVariantRow({ variant, onAdjustStock, disabled }: { variant: BackofficeCatalogProduct["variants"][number]; onAdjustStock: (sku: string, delta: number) => Promise<void>; disabled: boolean }) {
  ...
  return <tr><td data-label="SKU"><strong>{variant.sku}</strong></td><td data-label="Variante">{variant.color} · {variant.size}</td><td data-label="Precio">{variant.price.toLocaleString("es-AR")} {variant.currency}</td><td data-label="Stock" className={variant.stock > 0 ? "positive" : "negative"}>{variant.stock > 0 ? `${variant.stock} disponibles` : "Sin stock"}</td><td data-label="Operar"><div className="inline-action"><input aria-label={`Ajuste de stock ${variant.sku}`} inputMode="numeric" value={delta} onChange={(event) => setDelta(event.target.value)} /><button disabled={disabled} onClick={() => void onAdjustStock(variant.sku, Number.parseInt(delta, 10))}>Aplicar</button></div></td></tr>;
}
...
function FeatureFlagView({ snapshot }: { snapshot: FeatureFlagSnapshot }) {
  return <div className="store-list">
    ...
    {snapshot.flags.length === 0 ? <p className="muted">No hay flags publicados.</p> : <div className="table-wrap"><table className="responsive-table"><thead><tr><th>Key</th><th>Estado</th><th>Alcance</th><th>Agentes/versiones</th></tr></thead><tbody>{snapshot.flags.map((flag) => <tr key={flag.key}><td data-label="Key"><strong>{flag.key}</strong></td><td data-label="Estado" className={flag.enabled && !flag.killSwitch ? "positive" : "negative"}>{flag.killSwitch ? "KILL SWITCH" : flag.enabled ? "ACTIVA" : "INACTIVA"}</td><td data-label="Alcance">{flag.environments.join(", ") || "todos"} · {flag.channels.join(", ") || "todos"}</td><td data-label="Agentes/versiones">{flag.agentIds.join(", ") || "todos"}{flag.agentVersions.length ? ` · v${flag.agentVersions.join(", v")}` : ""}</td></tr>)}</tbody></table></div>}
...
function AgentRegistryView({ agents }: { agents: AgentRegistryAgent[] | null }) {
  ...
  return <div className="registry-list">{agents.map((agent) => <article className="registry-agent" key={agent.agentId}>
    ...
    <div className="table-wrap"><table className="responsive-table"><thead><tr><th>Versión</th><th>Lifecycle</th><th>Modelo</th><th>Prompt</th><th>Límites</th></tr></thead><tbody>{agent.versions.map((version) => <tr key={`${version.agentId}-${version.version}`}><td data-label="Versión"><strong>v{version.version}</strong><small>{version.name}</small></td><td data-label="Lifecycle">{version.state}<small>{version.evaluationSuiteVersion}</small></td><td data-label="Modelo">{version.modelProvider}<small>{version.modelId}</small></td><td data-label="Prompt">{version.systemPromptVersion}<small>SHA-256: {version.systemPromptHash.slice(0, 12)}…</small></td><td data-label="Límites">{version.maxSteps} steps<small>{version.maxInputTokens}/{version.maxOutputTokens} tokens</small></td></tr>)}</tbody></table></div>
    <h4>Activaciones</h4>
